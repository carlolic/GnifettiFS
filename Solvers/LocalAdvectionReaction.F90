...
!------------------------------------------------------------------------------
     ...
     LOGICAL :: DG,UseBubbles,Stabilize
     ...
     REAL(KIND=dp), ALLOCATABLE :: MASS(:,:), STIFF(:,:), LOAD(:), &
              ...
              UpperLimit(:), LowerLimit(:),TimeForce(:)
    ...
    ! Stabilisation method
     ...
     IF ( .NOT. AllocationsDone ) THEN
        ...
        ALLOCATE( FORCE(N), MASS(n,n), STIFF(N,N), LOAD(N),  &
                  ...
                  UpperLimit(n), LowerLimit(n), TimeForce(N), STAT = istat )
     ...
     DO iter=1,NonlinearIterMax
        ...
        DO t = 1, Active  
           ...
           CALL LocalMatrix( MASS, STIFF, FORCE, LOAD, Velo, MeshVelo, Gamma, Element, n , Stabilize, UseBubbles) 
           IF ( Transient ) CALL Default1stOrderTime( MASS, STIFF, FORCE )
           
           TimeForce  = 0.0_dp
           IF (UseBubbles) CALL Condensate( N, STIFF, FORCE, TimeForce )
        ...
        IF (Stabilize) CALL DefaultDirichletBCs()
...
!------------------------------------------------------------------------------      
     SUBROUTINE LocalMatrix(MASS, STIFF, FORCE, LOAD, Velo, Mvelo, Gamma, Element, n, Stabilize, UseBubbles)
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
       REAL(KIND=dp) :: Basis(2*n),dBasisdx(2*n,3),SW(2*n)
       REAL(KIND=dp) :: detJ,U,V,W,S,A,L,cu(3),g,divMVelo,hK,VNorm,Tau,Tau1,Tau2,r
       LOGICAL :: Stat
       LOGICAL :: UpWind
       INTEGER :: i,p,q,t,dim
       INTEGER :: NBasis
...
!------------------------------------------------------------------------------
       ...
       ! Upwind stabilisation if not bubbles
       UpWind=(Stabilize.AND.(.NOT.UseBubbles))
...
!------------------------------------------------------------------------------
         stat = ElementInfo( Element, Nodes, U, V, W, detJ, &
                 Basis, dBasisdx , Bubbles=UseBubbles )
         ...
         DO i=1,dim
            ...
            IF ( Stabilize ) divMVelo = divMVelo + SUM( Velo(i,1:n) * dBasisdx(1:n,i) )
         END DO
         ...
         IF (Upwind) THEN
            ! STAB PARAMETER 
            !(SEE Akin and Tezduyar, Calculation of the advective limit ..., 
            !  Comput. Methods Appl. Mech. Engrg. 193 (2004))
            Tau=0._dp
            !1/Tau1
            Tau1=0._dp
            Do i=1,n
               Tau1=Tau1+ABS(SUM(dBasisdx(i,1:dim)*cu(1:dim)))
            End do
            !1/Tau2
            Tau2=0._dp
            IF (Transient) Tau2=2.0/dt
            !Tau=(1/Tau1^r+1/tau2^r)^(-1/r) !r=2 here
            r=2.0
            Tau=Tau1**r+Tau2**r
         ...
         DO p=1,NBasis
            SW(p)=Basis(p)
            IF (Upwind) THEN
             ! cu or Velocity if ALE???
              DO i=1,dim
                 SW(p)=SW(p)+ Tau * cu(i) * dBasisdx(p,i)
              END DO
            END IF
         END DO
!------------------------------------------------------------------------------
...
!------------------------------------------------------------------------------
         DO p=1,NBasis
            DO q=1,NBasis
              MASS(p,q)  = MASS(p,q)  + s * Basis(q) * SW(p)
              STIFF(p,q) = STIFF(p,q) + s * (g + divMVelo) * Basis(q) * SW(p)
              DO i=1,dim
                IF (Stabilize) THEN
                  STIFF(p,q) = STIFF(p,q) + s * cu(i) * SW(p) * dBasisdx(q,i)
                ELSE
            ...
            END DO
         END DO
         FORCE(1:NBasis) = FORCE(1:NBasis) + s*L*SW(1:NBasis)
!------------------------------------------------------------------------------
      END DO
!------------------------------------------------------------------------------
